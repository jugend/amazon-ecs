...
Amazon ECS is a generic Product Advertising Ruby API. You can easily extend the library to support 
any of the REST operations.

The library wraps around Nokogiri element object, and it provides access to the XML response structure through XML path,
instead of object attribute.

The idea is the API will be constantly updated, and there will be changes to the XML schema, with Amazon ECS 
you would not have to update the library as the API evolves.
...
Thanks to Dan Milne and Bryan Houselfor the pull requests.
